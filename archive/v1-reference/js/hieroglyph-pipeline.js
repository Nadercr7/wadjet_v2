/**
 * ═══════════════════════════════════════════════════════════════
 *  Wadjet AI — Hieroglyph Browser Pipeline
 *  H6.3: Client-side detection (ONNX) + classification (TF.js)
 *  H6.5: Browser-side transliteration (Gardiner mapping)
 * ═══════════════════════════════════════════════════════════════
 *
 *  Stages (all client-side except translation):
 *    1. Detection:        ONNX Runtime Web → bounding boxes
 *    2. Classification:   TF.js → Gardiner codes
 *    3. Transliteration:  JS → MdC notation
 *    4. Translation:      Server API (POST /api/v1/hieroglyphs/translate)
 *
 *  Public API:
 *    new HieroglyphPipeline(options)
 *    .init()             → Promise<void>           — load all models
 *    .processImage(src)  → Promise<PipelineResult>  — full pipeline
 *    .detect(tensor)     → Promise<Detection[]>     — stage 1 only
 *    .classify(crops)    → Promise<Classification[]> — stage 2 only
 *    .transliterate(dets)→ TranslitResult            — stage 3 only
 *    .translate(mdc)     → Promise<TranslateResult>  — stage 4 (server)
 ...
 * @param {string} [opts.classifierUrl]   — TF.js model.json URL
 * @param {string} [opts.labelMapUrl]     — label_mapping.json URL
 * @param {string} [opts.translationApi]  — Server endpoint for translation
 * @param {number} [opts.detConfThreshold]— Detection confidence (default: 0.25)
...
function HieroglyphPipeline(opts) {
    ...
    // URLs
    this._detectorUrl    = opts.detectorUrl    || '/static/hieroglyph/models/detection/glyph_detector_uint8.onnx';
    this._classifierUrl  = opts.classifierUrl  || '/static/hieroglyph/models/tfjs_uint8/model.json';
    this._labelMapUrl    = opts.labelMapUrl    || '/static/hieroglyph/models/label_mapping.json';
    this._translationApi = opts.translationApi || '/api/v1/hieroglyphs/translate-inscription';

    // Config
    this._detConf    = opts.detConfThreshold || 0.25;
...
/**
 * Load all models. Call once before processImage.
...
HieroglyphPipeline.prototype.init = async function() {
    ...
    try {
        ...
        this._onnxSession = await ort.InferenceSession.create(this._detectorUrl, {
            executionProviders: ['wasm'],
            graphOptimizationLevel: 'all'
        });
        this._onProgress(0.5);

        // 3. Load TF.js classification model
        if (typeof tf === 'undefined') {
            throw new Error('TensorFlow.js not loaded. Add @tensorflow/tfjs CDN.');
        }
        this._tfModel = await tf.loadGraphModel(this._classifierUrl);

        // Auto-detect input size from model
        var inputShape = this._tfModel.inputs[0].shape;
        if (inputShape && inputShape[1]) {
            this._classSize = inputShape[1];
        }
...
HieroglyphPipeline.prototype._warmup = async function() {
    ...
    var inputName = this._onnxSession.inputNames[0];
    var feeds = {};
    feeds[inputName] = dummyDet;
    await this._onnxSession.run(feeds);

    // Warmup classification
    var dummyCls = tf.zeros([1, this._classSize, this._classSize, 3]);
    var pred = this._tfModel.predict(dummyCls);
    pred.dispose();
    dummyCls.dispose();
};
...
/**
 * Detect hieroglyph bounding boxes in an image.
 * @param {HTMLImageElement|HTMLCanvasElement|HTMLVideoElement} source
 * @returns {Promise<{boxes: Array, preprocessInfo: Object}>}
 */
HieroglyphPipeline.prototype.detect = async function(source) {
    ...
    for (var i = 0; i < pixelCount; i++) {
        floats[i]                  = imgData[i * 4]     / 255.0; // R → channel 0
        floats[pixelCount + i]     = imgData[i * 4 + 1] / 255.0; // G → channel 1
        floats[2 * pixelCount + i] = imgData[i * 4 + 2] / 255.0; // B → channel 2
    }
    ...
    var output = await this._onnxSession.run(feeds);
    var outputName = this._onnxSession.outputNames[0];
    var rawOutput = output[outputName];

    // Parse YOLO output: [1, 5, 8400] → after transpose: [8400, 5] (cx, cy, w, h, conf)
    var data = rawOutput.data;
    var numBoxes = rawOutput.dims[2]; // 8400
    var numCols = rawOutput.dims[1];  // 5

    var boxes = [];
    for (var b = 0; b < numBoxes; b++) {
        var conf = data[4 * numBoxes + b]; // Transposed: row 4 = confidence
...
/**
 * Classify detected glyph crops.
 * @param {HTMLImageElement|HTMLCanvasElement|HTMLVideoElement} source — original image
 * @param {Array} boxes — from detect()
 * @returns {Promise<Array>} — [{...box, classId, gardinerCode, classConf}]
 */
HieroglyphPipeline.prototype.classify = async function(source, boxes) {
    ...
    var results = [];
    var i2g = this._labelMap.idx_to_gardiner;

    // Process in batch using TF.js
    var tensors = tf.tidy(function() {
        ...
        for (var i = 0; i < boxes.length; i++) {
            ...
            var tensor = tf.browser.fromPixels(cropCanvas)
                .toFloat()
                .div(255.0);
            crops.push(tensor);
        }
        return tf.stack(crops); // [N, H, W, 3]
    });

    var logits = this._tfModel.predict(tensors);
    var probs  = tf.softmax(logits, -1);
    var classIds = tf.argMax(probs, -1);
    var maxProbs = tf.max(probs, -1);

    var idsArr   = await classIds.data();
    var probsArr = await maxProbs.data();

    for (var i = 0; i < boxes.length; i++) {
        var classId = idsArr[i];
        var gardiner = i2g[String(classId)] || ('UNK_' + classId);
        results.push({
            x1: boxes[i].x1,
            y1: boxes[i].y1,
            x2: boxes[i].x2,
...
/**
 * Convert classified glyph sequence to MdC transliteration.
 * Uses simplified reading order (left-to-right sorted by x-position).
 * @param {Array} glyphs — from classify()
 * @returns {Object} — {transliteration, gardinerSequence, direction}
 */
HieroglyphPipeline.prototype.transliterate = function(glyphs) {
    ...
    // Sort by reading order: cluster into lines (by y), sort within line (by x, RTL default)
    var lines = this._clusterIntoLines(glyphs);
    var direction = 'RTL'; // Default for Egyptian
...
/**
 * Cluster glyphs into lines by vertical overlap.
 */
...
/**
 * Send transliteration to server for translation.
 * @param {string} mdc — MdC transliteration
 * @returns {Promise<{english: string, arabic: string, error: string}>}
 */
...
/**
 * Run the complete pipeline on an image source.
 * @param {HTMLImageElement|HTMLCanvasElement|HTMLVideoElement} source
 * @param {Object} [opts]
 * @param {boolean} [opts.translate=false] — include server-side translation
...
/**
 * Build static Gardiner → transliteration mapping.
 * Covers uniliterals, common biliterals, and determinative categories.
 */
HieroglyphPipeline.prototype._buildGardinerMap = function() {
    ...
    var uni = {
        'G1':'A','M17':'i','M17A':'i','D36':'a','G43':'w',
        'D58':'b','Q3':'p','I9':'f','G17':'m','N35':'n',
        'D21':'r','O4':'h','V28':'H','Aa1':'x','Aa2':'X',
        'S29':'s','N37':'S','D46':'d','I10':'D','X1':'t',
        'V13':'T','N29':'q','V31':'k','W11':'g'
    };
    for (var code in uni) {
    ...
    // Common biliterals
    var bi = {
        'Y5':'mn','R4':'Htp','U1':'mA','P8':'xn','F35':'nfr',
        'S42':'wAH','U7':'mr','W14':'Hz','D28':'kA','V30':'nb',
        'Aa15':'wDa','F34':'ib','T14':'aHA','F12':'wsr','O34':'zA',
        'M3':'Ht','U15':'Hm','M8':'SA','R7':'sn','D4':'ir',
        'U6':'mr','V29':'wAD','M41':'Hsa','D53':'mw','N25':'xAs',
        'U23':'Ab','M12':'sp','M23':'sw','D2':'Hr','O1':'pr',
        'N5':'ra','T19':'Dba','D19':'fnD','R8':'nTr','M16':'HA',
        'M44':'Sma','D52':'mt','S34':'anx','V4':'ms','F32':'bA',
        'U28':'Dd','Aa28':'qd','Aa26':'Hs','Aa27':'nd'
    };
    for (var code in bi) {
    ...
    // Common triliterals
    var tri = {
        'R11':'Dd','G25':'Ax','F40':'Aw','S38':'mDH',
        'F4':'nDm','R19':'wAb','G7':'bik'
    };
    for (var code in tri) {
    ...
    // Common logograms
    var logo = {
        'N1':'tA','N14':'sbA','N26':'Dw','N36':'mr',
        'N16':'tA','D1':'tp','D56':'rd','D60':'sxm',
        'F44':'spd','G5':'Hr','N31':'wAt','O6':'Hwt',
        'P1':'dpt','R22':'xnt'
    };
    for (var code in logo) {
        m[code] = { transliteration: logo[code], isDeterminative: false };
    }

    // Determinatives
    var det = {
        'A1':'man','A2':'man','B1':'woman','D40':'action',
        'G37':'bird','M1':'tree','N33':'grain','O49':'city',
        'Y1':'abstract','Z1':'stroke','A14':'death','D54':'walking',
        'A24':'striking','E1':'animal','F27':'skin','F51':'flesh',
        'N23':'land','N35A':'water','P1A':'boat','T30':'weapon',
        'W22':'vessel','Z2':'plural'
    };
    for (var code in det) {
...
HieroglyphPipeline.isAvailable = function() {
    return typeof tf !== 'undefined' && typeof ort !== 'undefined';
};
